...
namespace ProcessingModule
{
    ...
    public class AutomationManager : IAutomationManager, IDisposable
	{
		...
		private void AutomationWorker_DoWork()
		{
			...
			List<PointIdentifier> pointList = new List<PointIdentifier> { analogOut, digitalOut1, digitalOut2, prepreka};
            
            bool preprekaAktivna = false;
			...
			while(!disposedValue)
			{
				...
				{
					value -= 10;
                    if (preprekaAktivna == true && value <= points[0].ConfigItem.LowLimit)
                    {
                        value = (int)points[0].ConfigItem.LowLimit;
                        processingManager.ExecuteWriteCommand(points[1].ConfigItem, configuration.GetTransactionId(), configuration.UnitAddress, 3000, 0);
                    }
                ...
                if (points[2].RawValue == 1)
                {
                    ...
                    if(points[3].RawValue == 1)
                    {
                        ...
                        preprekaAktivna = true;
                    }
                }

                if (preprekaAktivna == true && points[3].RawValue == 0 && points[1].RawValue == 0)
                {
